...
static void SNTPToTimeval(double millisecs, struct timeval *Time)
{
    ...
    if (Args->Flags & FLAG_VERBOSE) printf("%ld.%04ld adrift\n", labs((long) diff / 1000), labs((long) diff % 1000));
    if (Args->Flags & FLAG_SYSLOG) syslog(LOG_INFO, "%ld.%04ld adrift", labs((long) diff / 1000), labs((long) diff % 1000));
}
...
// This is the function that connects to a server and requests time
int SNTPGetTime(const char *Host, int Port, struct timeval *Time)
{
...
//host is a const char ptr so we don't use CopyStr
    if (! StrValid(Host)) Host="pool.ntp.org";
    if (Port==0) Port=123;

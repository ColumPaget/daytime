...
void PrintUsage()
{
    ...
    fprintf(stdout,"\n");
    printf("Usage:\n");
printf("	daytime -daytime|-time|-nist|-http|-sntp|-ssh <server> [-s] [-r] [-tz <timezone>] [-P <path>]\n");
    printf("Note, <server> option is optional for -nist, -ntp, -sntp, -http as if none is supplied a default is used\n\n");
    printf("	daytime <url> [-s] [-r] [-tz <timezone>] [-P <path>]\n");
    printf("Supported url types are http: https: ssh: ntp: sntp: time: daytime:\n\n");
    ...
    printf("	-nist		Get time from NIST daytime server, (default time-a.nist.gov)\n");
    printf("	-http		Get time from a web server (default www.google.com)\n");
    printf("	-sntp		Get time from a SNTP server (default pool.ntp.org). (Not full NTP, only accurate to seconds). If server arg is 'bcast' then wait to recieve sntp broadcast.\n");
    printf("	-ntp 		Get time from a SNTP server (default pool.ntp.org). (Not full NTP, only accurate to seconds). If server arg is 'bcast' then wait to recieve sntp broadcast.\n");
...
void CommandLineParseTimeSource(TArgs *Args, const char *Source)
{
    char *Tempstr=NULL, *Token=NULL;

    ParseURL(Source, &Tempstr, NULL, &Token, NULL, NULL, NULL, NULL);
    if (StrValid(Tempstr)) Args->URL=CopyStr(Args->URL, Source);
    else
    {
        if (Args->Flags & FLAG_HTTP) Args->URL=MCopyStr(Args->URL, "http:", Source, NULL);
        else if (Args->Flags & FLAG_SSH)
        {
            if (! StrValid(Token)) Token=CopyStr(Token, "22");
            Args->URL=MCopyStr(Args->URL, "ssh:", Source, ":", Token, NULL);
        }
        else if (Args->Flags & FLAG_SNTP)
        {
            if (! StrValid(Token)) Token=CopyStr(Token, "123");
            Args->URL=MCopyStr(Args->URL, "sntp:", Source, ":", Token, NULL);
        }
        else if (Args->Flags & FLAG_DAYTIME)
        {
            if (! StrValid(Token)) Token=CopyStr(Token, "13");
            Args->URL=MCopyStr(Args->URL, "daytime:", Source, ":", Token, NULL);
        }
        else if (Args->Flags & FLAG_TIME)
        {
            if (! StrValid(Token)) Token=CopyStr(Token, "37");
            Args->URL=MCopyStr(Args->URL, "time:", Source, ":", Token, NULL);
        }
        else if (Args->Flags & FLAG_NIST)
        {
            if (! StrValid(Token)) Token=CopyStr(Token, "13");
            Args->URL=MCopyStr(Args->URL, "nist:", Source, ":", Token, NULL);
        }
    }
...
TArgs *CommandLineParse(int argc, char *argv[])
{
    ...
    for (i=1; i < argc; i++)
    {
        ...
        {
            Args->Flags |= FLAG_TIME;
            Args->URL=CopyStr(Args->URL, "time:time-a.nist.gov:37");
        }
        else if (strcmp(argv[i],"-nist")==0)
        {
            Args->Flags |= FLAG_NIST;
            Args->URL=CopyStr(Args->URL, "nist:time-a.nist.gov:13");
        }
        else if (strcmp(argv[i],"-http")==0)
        {
            Args->Flags |= FLAG_HTTP;
            Args->URL=CopyStr(Args->URL, "http:www.google.com");
        }
        else if (strcmp(argv[i],"-ntp")==0)
        {
            Args->Flags |= FLAG_SNTP;
            Args->URL=CopyStr(Args->URL, "sntp:pool.ntp.org:123");
        }
        else if (strcmp(argv[i],"-sntp")==0)
        {
            Args->Flags |= FLAG_SNTP;
            Args->URL=CopyStr(Args->URL, "sntp:pool.ntp.org:123");
        }
        else if (strcmp(argv[i],"-ssh")==0) Args->Flags |= FLAG_SSH;
        ...
        else if (strcmp(argv[i],"-tz")==0) RemoteTimeZone=CopyStr(RemoteTimeZone,argv[++i]);
        /*
        ...
        */
        ...
        else CommandLineParseTimeSource(Args, argv[i]);
